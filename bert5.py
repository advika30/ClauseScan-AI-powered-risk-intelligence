...
def save_user(username, password):
    ...
    signup_time = datetime.datetime.utcnow() + datetime.timedelta(hours=5, minutes=30)
    collection.insert_one({
        "username": username, 
        "hashed_password": hashed, 
        "signup_timestamp": signup_time,
        "login_history": []  # Initialize empty login history
    })
...
def authenticate_user(username, password):
    ...
    if user and user["hashed_password"] == hash_password(password):
        ...
        login_time = datetime.datetime.utcnow() + datetime.timedelta(hours=5, minutes=30)
